...
export class PluginOAuth implements OAuthClientProvider {
  ...
  get clientMetadata() {
    return { client_name: 'Octo Chat', redirect_uris: [this.redirectUrl], grant_types: ['authorization_code', 'refresh_token'], response_types: ['code'], token_endpoint_auth_method: 'none' };
  }
    ...
    try {
      ...
      server.on('request', (request, response) => {
        ...
        if (url.searchParams.has('error') || !url.searchParams.get('code')) {
          response.writeHead(400).end('Sign-in was not completed. Return to Octo Chat.');
          fail(new Error('Plugin sign-in was not completed.')); return;
        }
        response.end('Authorization received. Return to Octo Chat.'); server.close(); settle(url.searchParams);
      });
