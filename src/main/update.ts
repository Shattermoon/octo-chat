/**
 * Whether a newer Octo Chat has been published, and the file that becomes it.
 *
...
const REPO = 'Shattermoon/octo-chat';
...
export function stagedArtifact(
  ...
  if (platform === 'win32') return { name: `Octo-Chat-Setup-${arch}.exe`, kind: 'installer', target: '' };
  if (platform === 'linux' && appImage) {
    return { name: `Octo-Chat-Linux-${arch}.AppImage`, kind: 'appimage', target: appImage };
  }
  return null;
}
...
async function get(url: string, timeout: number, headers: Record<string, string> = {}): Promise<Response> {
  const response = await fetch(url, {
    ...
    redirect: 'follow',
    headers: { 'user-agent': `octo-chat/${APP_VERSION}`, ...headers }
  });
