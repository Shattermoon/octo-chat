...
import { authorizeToolAction, dispatch, fail, type ToolContext, type ToolResult } from './kernel.js';
...
async function runPluginTool(name: string, args: unknown, liveContext: () => ToolContext): Promise<ToolResult> {
  const policy = authorizeToolAction(liveContext(), 'plugins', name, { kind: 'none', denyInReadOnly: true });
  if (policy.effect === 'deny') return pluginManager.redactResult(fail('TOOL_DISABLED: external plugins are unavailable while Octo Chat read-only mode is on.')) as ToolResult;
 ...
 */
export function registerPluginTools(server: McpServer, liveContext: () => ToolContext): PluginToolSchema[] {
  ...
  const runCode = codeModeHandler(() => tools.map(tool => ({ name: tool.name, description: tool.description ?? '' })),
    (name, args, parent) => dispatch(name, args, parent.caller.transportKey, parent.caller.requestId, 'plugins', () => runPluginTool(name, args, liveContext), parent));
  ...
  server.server.setRequestHandler('tools/call', async (request, context) => {
    ...
    const result = await dispatch(request.params.name, request.params.arguments ?? {}, context.sessionId ?? null,
      inboundRequestId(), 'plugins', async () => {
        ...
        return runPluginTool(request.params.name, request.params.arguments ?? {}, liveContext);
      });
