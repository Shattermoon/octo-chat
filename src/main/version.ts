...
export function extensionDownloadUrl(version = APP_VERSION): string {
  return `https://github.com/Shattermoon/octo-chat/releases/download/v${encodeURIComponent(version)}/Octo-Chat-Extension.zip`;
}
 ...
 * 7 — the app renamed itself to Octo Chat, and the `app` field every bridge response
...
// 14 — Octo Chat changed the bridge application identity to `octo-chat`. An older companion
// expects the retired identity and would otherwise discard replies as if no app answered.
