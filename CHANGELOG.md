...
## [2.2.0] — Octo Chat public baseline

- Renamed the product, package, connectors, artifacts, updater, extension and public repository to **Octo Chat**.
- Republished the project from a clean public root commit; earlier incorporated community work remains credited in `CONTRIBUTORS.md`.
- Moved the canonical repository to `Shattermoon/octo-chat` and updated release/update URLs accordingly.
- Bumped the companion bridge to protocol 14 so old-brand companions fail clearly instead of silently discarding the new `octo-chat` bridge identity.
- Paused macOS support and removed macOS from current CI, packaging and release publication. Windows and Linux remain the supported release platforms; native Desktop automation is currently Windows-only.
...
- **Linux desktop identity is explicit.** `desktopName` and `StartupWMClass` now share the
  `com.octochat.app` identity so installed launchers can associate the running Electron window
...
### Fixed
- **Sub-agents can be enabled without restarting Octo Chat.** Swarm persistence hooks are
...
### Changed
- **The app is now called Octo Chat.** The installer, the window, the connector
  ...
  folder (`%APPDATA%\octo-chat\`), so the stored API key and recorded sessions
...
[1.9.5]: https://github.com/Shattermoon/octo-chat/releases/tag/v1.9.5
[1.9.4]: https://github.com/Shattermoon/octo-chat/releases/tag/v1.9.4
