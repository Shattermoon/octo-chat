...
const app = process.argv[3]
  ...
  : path.join(releaseDir, unpackedDir, 'Octo Chat.app');
...
const expectedPlist = {
  CFBundleIdentifier: 'com.octochat.app',
  CFBundleExecutable: 'Octo Chat',
  CFBundleName: 'Octo Chat',
  CFBundleDisplayName: 'Octo Chat',
  ...
  NSScreenCaptureUsageDescription:
    'Octo Chat captures a display or window only when the enabled Desktop connector asks to observe it.'
};
...
function isLaunchedMachO(file) {
  ...
  return (
    ...
    path.basename(file) === 'libocto-desktop.dylib'
  );
}

const nativeDir = `darwin-${arch}`;
const nodeModules = path.join(resources, 'app.asar.unpacked', 'node_modules');
const mainExecutable = path.join(contents, 'MacOS', 'Octo Chat');
...
const desktopLibrary = path.join(resources, 'desktop', 'libocto-desktop.dylib');
