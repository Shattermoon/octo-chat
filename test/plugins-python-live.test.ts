...
it.runIf(process.env.OCTO_PLUGIN_LIVE_TEST === '1')('installs the reviewed Fetch release and discovers its real server', async () => {
  const temporary = await makeTempDir('octo-python-plugin-live-');
  ...
  try {
    ...
    expect(JSON.stringify(result)).toContain('Octo Chat Python fetch fixture');
  } finally {
    await manager.close();
