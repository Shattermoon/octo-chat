...
function workflow(options: { unchanged?: boolean; deny?: boolean; navigateDuringClaim?: boolean; refreshAvailable?: boolean } = {}) {
  ...
  const context = vm.createContext({ URL, alive: true, generating: false, epoch: 1,
    location: { pathname: '/', href: `https://chatgpt.com/?octo-plugin-refresh=${id}#settings/Plugins/plugin_asdk_app_synthetic` },
  ...
  return { click, ask, run: () => (context.run as Function)({ id, appId: 'asdk_app_synthetic', connectorName: 'Octo Chat Core', tools }) };
}
...
it.each([false, true])('waits for readable tool schemas before claim (navigation=%s)', async navigate => {
  ...
  const pending = (context.run as Function)({ id, appId: null, connectorName: 'Octo Chat Core', tools });
...
it('keeps a loading settings index pending and restores custody after its installed button drops the marker', async () => {
  ...
  const location = { pathname: '/', href: `https://chatgpt.com/?octo-plugin-refresh=${id}#settings/Plugins`, replace };
  ...
  vm.runInContext(`${section}\nwaitPageView = async (read, current) => current() ? read() : null; globalThis.run = refreshManagedPlugin;`, context);
  const run = () => (context.run as Function)({ id, appId: null, connectorName: 'Octo Chat Core', tools });
  ...
  expect(replace).toHaveBeenCalledExactlyOnceWith(undefined, '', `https://chatgpt.com/?octo-plugin-refresh=${id}#settings/Plugins/plugin_asdk_app_synthetic`);
...
it('opens an enrolled exact App Id directly in marked settings without name discovery', async () => {
  ...
  expect(create).toHaveBeenCalledExactlyOnceWith(`https://chatgpt.com/?octo-plugin-refresh=${id}#settings/Plugins/plugin_asdk_app_synthetic`, true);
});
...
it.each(['unpinned', 'pinned', 'pinned-during-proof'])('reuses management tabs and respects pinning (%s)', async mode => {
  ...
  const tabs = [{ id: 7, url: `https://chatgpt.com/?octo-plugin-refresh=${id}#settings/Plugins`, pinned: false }, { id: 8, url: 'https://chatgpt.com/c/user-conversation', pinned: false }];
  const create = vi.fn(async () => ({ id: 9 }));
  const remove = vi.fn();
