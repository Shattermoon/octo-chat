...
const safeEmail = '155615649+nkcbuilds@users.noreply.github.com';
...
function commit(repository: string, message: string, email: string): void {
  execFileSync('git', ['commit', '--allow-empty', '-m', message], {
    ...
    env: {
      ...process.env,
      GIT_AUTHOR_NAME: 'nkcbuilds',
      GIT_AUTHOR_EMAIL: email,
      GIT_COMMITTER_NAME: 'nkcbuilds',
...
function verify(repository: string, args: string[] = []) {
  return spawnSync(process.execPath, [script, ...args], {
    ...
    encoding: 'utf8',
    windowsHide: true,
    env: { ...process.env, GIT_AUTHOR_NAME: 'nkcbuilds', GIT_AUTHOR_EMAIL: safeEmail },
  });
}
...
describe('public-history privacy gate', () => {
  ...
  it.each([
    'https://github.com/Shattermoon/octo-chat.git',
    'git@github.com:Shattermoon/octo-chat.git',
    'ssh://git@github.com/Shattermoon/octo-chat'
  ...
  it.each([
    'https://github.com/example/octo-chat.git',
    'https://github.com/Shattermoon/octo-chat-extra.git',
    'https://github.com.example/Shattermoon/octo-chat.git'
  ...
  it('does not fall back to fork history when canonical main has not been fetched', () => {
    const repository = makeRepository();
    execFileSync('git', ['remote', 'add', 'upstream', 'https://github.com/Shattermoon/octo-chat.git'], { cwd: repository });
