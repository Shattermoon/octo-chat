...
describe('browser-backed ChatGPT commands', () => {
  it('launches selected Edge when Chrome is also installed', async () => {
    ...
    const url = 'https://chatgpt.com/?octo-model-catalog=selected';
  ...
  it('cold background startup gives Chrome one owned tab in a minimized startup window', async () => {
    ...
    await openInPreferredBrowser('https://chatgpt.com/?octo-model-catalog=owned', {
    ...
    expect(calls).toEqual([`$ErrorActionPreference='Stop'; Start-Process -FilePath '${browser}' -ArgumentList '"--disable-renderer-backgrounding" "--disable-background-timer-throttling" "--window-size=800,600" "https://chatgpt.com/?octo-model-catalog=owned"' -WorkingDirectory '${path.win32.dirname(browser)}' -WindowStyle Minimized`]);
    expect(launch).not.toHaveBeenCalled();
  });
  ...
  it.each([{ exitCode: 1, timedOut: false }, { exitCode: null, timedOut: true }])('reports minimized startup failure without direct foreground fallback: %j', async failure => {
    const launch = vi.fn();
    const browser = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
    await expect(openInPreferredBrowser('https://chatgpt.com/?octo-model-catalog=owned', {
      platform: 'win32', env: { ProgramFiles: 'C:\\Program Files' }, backgroundStartup: true,
