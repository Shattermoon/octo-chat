...
describe('cross-platform product surface', () => {
  it('treats Darwin as unsupported while masking any stored Desktop grants', () => {
    ...
    expect(effectiveCapabilities(switchedOn, 'darwin', '21.4.0').screen).toBe(false);
    expect(surfaceIsUseful('desktop', switchedOn.capabilities, 'darwin', '21.4.0')).toBe(false);
  });
