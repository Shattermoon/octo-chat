...
const context = { electronPlatformName: 'darwin', appOutDir: '/package', packager: { appInfo: { productFilename: 'Octo Chat' } } };
beforeEach(() => {
  vi.resetAllMocks();
  ports.exists.mockReturnValue(true);
  ports.spawn.mockImplementation((_command: string, args: string[]) => ({ status: 0, stdout: '',
    stderr: args.includes('--display') ? 'Identifier=com.octochat.app\nSignature=adhoc\nTeamIdentifier=not set\n' : '' }));
});
