...
describe('the user’s own connector instructions', () => {
  it('starts with the coding guidance and explains connectors once beside the local tools without a setup link', () => {
    ...
    expect(text.startsWith('You are a coding agent working with the user through Octo Chat.')).toBe(true);
    const intro = text.split('\n').find(line => line.startsWith('Use the connected tools as needed:'))!;
    expect(intro).toContain('Octo Chat Core for files');
    expect(intro).toContain('Octo Chat Desktop for screen');
    expect(intro).toContain('Octo Chat Plugins for enabled external apps');
    expect(text.indexOf(intro)).toBeGreaterThan(text.indexOf('# Local tools'));
    expect(text).not.toMatch(/This is Octo Chat|https:\/\/chatgpt.com\/#settings\/Plugins/);
    expect(serverInstructions(ctx, 'core', 'linux')).not.toContain('Octo Chat Desktop');
  });
