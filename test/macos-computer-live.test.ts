...
const LIVE = process.platform === 'darwin' && process.env['OCTO_LIVE_MAOCTO_DESKTOP'] === '1';
const SEMANTIC_PROCESS = process.env['OCTO_LIVE_MAOCTO_SEMANTIC_PROCESS'];
const SEMANTIC_WINDOW = process.env['OCTO_LIVE_MAOCTO_SEMANTIC_WINDOW'];
const SEMANTIC_QUERY = process.env['OCTO_LIVE_MAOCTO_SEMANTIC_QUERY'];
...
describe.runIf(LIVE)('live macOS Desktop backend', () => {
  ...
  it('sets and clears an explicitly selected semantic text control', async (context) => {
    if (!SEMANTIC_PROCESS || !SEMANTIC_QUERY) {
      return context.skip('Set OCTO_LIVE_MAOCTO_SEMANTIC_PROCESS and OCTO_LIVE_MAOCTO_SEMANTIC_QUERY to opt in.');
    }
    ...
    const marker = 'OCTO_MAC_DESKTOP_SEMANTIC_PROBE';
    try {
