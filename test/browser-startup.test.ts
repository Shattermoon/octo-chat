...
it('starts the newly selected family without reusing the old attempt or overriding a connected companion', async () => {
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=old');
  config.ui.chatBrowser = 'edge';
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=new');
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=repeat');
  expect(open).toHaveBeenCalledTimes(2);
  browser.connected = true;
  config.ui.chatBrowser = 'chrome';
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=connected');
  expect(open).toHaveBeenCalledTimes(2);
});
...
it('discards absence evidence when the selected browser changes during its probe', async () => {
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=old', true);
  expect(open).not.toHaveBeenCalled();
  running.mockResolvedValue(false);
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=new', true);
...
it('uses confirmed process absence to supersede stale HTTP presence, never socket suspension alone', async () => {
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=one', true);
  expect(open).not.toHaveBeenCalled();
  running.mockResolvedValue(false);
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=one', true);
  expect(open).toHaveBeenCalledTimes(1);
...
it('shares concurrent absence probes and never opens after the wake socket reconnects during a probe', async () => {
  ...
  const first = wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=new', true);
  const second = wakeBrowserUrl('https://chatgpt.com/?octo-input=other', true);
  await Promise.resolve(); release(false);
  await Promise.all([first, second]);
  expect(open).toHaveBeenCalledTimes(2);
  running.mockImplementation(async () => { browser.connected = true; return false; });
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=third', true);
  expect(open).toHaveBeenCalledTimes(2);
});
it('also opens for a first authored send when Chrome exited inside the HTTP presence window', async () => {
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-input=first');
  expect(open).toHaveBeenCalledTimes(1);
});

it('shares one successful absence episode across input and discovery retries', async () => {
  await Promise.all([wakeBrowserUrl('https://chatgpt.com/?octo-input=one'), wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=two', true)]);
  running.mockResolvedValue(true);
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=three', true);
  expect(open).toHaveBeenCalledTimes(1);
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=four', true);
  expect(open).toHaveBeenCalledTimes(2);
});
...
it('waits for real absence after the wake channel closes with recent HTTP presence', async () => {
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=discovery');
  expect(open).not.toHaveBeenCalled();
  running.mockResolvedValue(true);
  browser.connected = false; // MV3 suspension or reconnect is not proof of browser absence
  await Promise.all([wakeBrowserUrl('https://chatgpt.com/?octo-input=first'), wakeBrowserUrl('https://chatgpt.com/?octo-input=second')]);
  expect(open).not.toHaveBeenCalled();
  browser.present = false; running.mockResolvedValue(false);
  await wakeBrowserUrl('https://chatgpt.com/?octo-input=first');
  expect(open).toHaveBeenCalledTimes(1);
  expect(open).toHaveBeenCalledWith('https://chatgpt.com/?octo-input=first');
});
it('starts model discovery without asking the OS to open its URL in a foreground window', async () => {
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=one', true, true);
  expect(open).toHaveBeenCalledWith('https://chatgpt.com/?octo-model-catalog=one', { backgroundStartup: true });
});
...
it('opens an authorized recovery only after proving process absence, including stale HTTP absence', async () => {
  ...
  await Promise.all([
    ...
    wakeBrowserUrl('https://chatgpt.com/?octo-input=concurrent')
  ]);
  expect(open).toHaveBeenCalledTimes(1);
});
...
it.each([true, null])('never forwards an initial discovery or authored URL to an existing or unknown process (%s)', async state => {
  ...
  await wakeBrowserUrl('https://chatgpt.com/?octo-model-catalog=first', true, true);
  await wakeBrowserUrl('https://chatgpt.com/?octo-input=first');
  expect(open).not.toHaveBeenCalled();
});
