...
function harness() {
  ...
  return {
    ...
    request: (source: unknown = window, origin = 'https://chatgpt.com') => dispatch('message', { source, origin, data: { type: 'octo-usage-request' } })
  };
}
...
describe('MAIN-world usage projection', () => {
  ...
  it('projects shared percentage windows without copying credentials or inventing a model balance', async () => {
    ...
    expect(h.posts).toEqual([{ type: 'octo-usage', observedAt: expect.any(Number), rows: [{ model: 'Shared usage', scope: 'shared', remaining: null, remainingPercent: 75, resetAt: 1900000000000, windowSeconds: 18000 }] }]);
  ...
  it('ignores foreign and unrelated responses, invalid counts and oversized payloads', async () => {
    ...
    expect(h.posts).toEqual([{ type: 'octo-usage', observedAt: expect.any(Number), rows: [] }]);
  });
  ...
  it('publishes an exact conversation/request pair from a chunked live response before React renders it', async () => {
    ...
    expect(h.posts).toEqual([
      { type: 'octo-request-origin', conversationId, requestIds: ['wfr_early_exact'], observedAt: expect.any(Number) },
      { type: 'octo-request-origin', conversationId, requestIds: ['wfr_second'], observedAt: expect.any(Number) }
    ]);
  ...
  it('reattaches after the page runtime replaces fetch during startup', async () => {
    ...
    expect(h.posts).toEqual([
      { type: 'octo-request-origin', conversationId, requestIds: ['wfr_after_runtime_wrap'], observedAt: expect.any(Number) }
    ]);
  });
  ...
  it('preserves a page wrapper that delegates to the earlier observer without recursion or duplicate inspection', async () => {
    ...
    expect(h.posts).toEqual([
      { type: 'octo-request-origin', conversationId, requestIds: ['wfr_nested_wrapper'], observedAt: expect.any(Number) }
    ]);
  });
