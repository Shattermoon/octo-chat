...
describe('Goal decision backends', () => {
  ...
  it('keeps all 200 template variants unique with identical marker contracts', () => {
    ...
    for (let index = 0; index < GOAL_CONTINUATIONS.length; index++) {
      expect(templateGoalDecision('[[OCTO_GOAL:CONTINUE]]', index)).toEqual({ action: 'continue', reply: GOAL_CONTINUATIONS[index] + ' ' + GOAL_CONTINUATIONS[(index + 1) % 200] + GOAL_MARKER_INSTRUCTION });
    }
    for (const text of ['prefix [[OCTO_GOAL:COMPLETE]]', '[[OCTO_GOAL:COMPLETE]]\nMore work', '[[OCTO_GOAL:complete]]']) {
