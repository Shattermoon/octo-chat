...
const NEXT = '99.0.0';
const WINDOWS_ASSET = `Octo-Chat-Setup-${process.arch}.exe`;
const APPIMAGE_ASSET = `Octo-Chat-Linux-${process.arch}.AppImage`;
...
beforeEach(() => {
  userData = mkdtempSync(path.join(tmpdir(), 'octo-update-'));
  packaged = true;
  spawned.length = 0;
  relaunched.length = 0;
  resetUpdateForTests();
...
describe('which installations update themselves', () => {
  it('takes the Windows installer and the Linux AppImage, and nothing else', () => {
    expect(stagedArtifact('win32', 'x64')).toMatchObject({ name: 'Octo-Chat-Setup-x64.exe', kind: 'installer' });
    expect(stagedArtifact('linux', 'arm64', '/opt/octo.AppImage')).toMatchObject({
      name: 'Octo-Chat-Linux-arm64.AppImage',
      kind: 'appimage',
      target: '/opt/octo.AppImage'
    });
  });
...
describe('staging the new version', () => {
  ...
  it('stages nothing when the release does not publish an artifact for this installation', async () => {
    github({ checksums: `${sha256('x')}  Octo-Chat-Extension.zip\n` });
    await asPlatform('win32', undefined, () => checkForUpdates());
    expect(updateStatus().stage).toBe('failed');
  ...
  it('replaces the running AppImage with the staged one', async () => {
    const live = path.join(userData, 'Octo-Chat.AppImage');
