...
async function run(script: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), 'octo-app-catalog-test-'));
  try {
...
$executable=Join-Path $PSScriptRoot 'octo-owned-app.exe'
...
if ((Resolve-AppExecutable 'octo-owned-app.exe') -cne $executable) { throw 'PATH executable did not resolve' }
foreach ($invalid in @('octo-owned-app.exe --flag','octo-owned-app.exe; command','.\octo-owned-app.exe','https://example.com/app.exe')) {
...
$result=Launch-WindowsApp @{app='octo-owned-app.exe'}
