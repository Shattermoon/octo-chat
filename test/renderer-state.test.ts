...
it('preserves optional Desktop disclosures and inline screenshots across status pushes', async () => {
  ...
  expect(doc.querySelectorAll('[data-setup-guide="plugin"] img')).toHaveLength(1);
  ...
  expect(image.src).toContain('create-tunnel.png');
...
it('says nothing about being current until the check has actually answered', async () => {
  ...
  expect(line.textContent).toBe('Up to date! Octo Chat 2.0.2 · extension 2.0.2');
