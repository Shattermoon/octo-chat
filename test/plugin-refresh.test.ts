...
const publish = (version = '1', declarations = tools) => { publishPluginSurface('core', 'Octo Chat Core', version, 'Instructions', declarations); vi.advanceTimersByTime(20_000); };
const publishPlugins = (declarations: PluginToolSchema[]) => { publishPluginSurface('plugins', 'Octo Chat Plugins', '1', 'Instructions', declarations); vi.advanceTimersByTime(20_000); };
const claim = (request: { id: string }, declarations = [{ ...tools[0]!, description: 'Older declaration' }]) => claimPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Core', tools: declarations });
it('debounces only changed declarations for twenty seconds and fences stale claims', async () => {
  ...
  const change = (description: string) => publishPluginSurface('core', 'Octo Chat Core', '1', 'Instructions', [{ ...tools[0]!, description }]);
  change('A');
...
it('keeps Desktop independent and debounces shape changes across reconnects', async () => {
  ...
  publishPluginSurface('core', 'Octo Chat Core', '1', '', [{ ...tools[0]!, description: 'Reconnected shape' }]);
...
it('wakes a settled publication restored after its deadline elapsed while disconnected', async () => {
  ...
  publishPluginSurface('core', 'Octo Chat Core', '1', '', changed);
  unpublishPluginSurface('core');
  vi.advanceTimersByTime(20_000);
...
it.each([{ count: 118, codeMode: false }, { count: 118, codeMode: true }, { count: 256, codeMode: true }])('enrolls legacy Plugins into $count tools with code mode $codeMode and requires the complete expanded catalog', async ({ count, codeMode }) => {
  ...
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Plugins', tools: legacy })).toBe(true);
...
it('rejects a foreign declaration inside a legacy Plugins subset', async () => {
  ...
  publishPlugins(catalog);
  const request = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Plugins', tools: legacy })).toBe(false);
});
it('enrolls already-current tools without granting a refresh click, including after restart', async () => {
  ...
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Core', tools, alreadyCurrent: true })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
  resetPluginRefreshForTests();
  publishPluginSurface('core', 'Octo Chat Core', 'a-new-app-version', 'Different runtime instructions', tools);
  expect(await pendingPluginRefreshes()).toEqual([]);
});
it('does not settle a changed contract as already current', async () => {
  publish(); const request = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Core', tools: [{ ...tools[0]!, description: 'Old' }], alreadyCurrent: true })).toBe(false);
  expect(await claim(request)).toBe(true);
});
it('durably suppresses automatic retries when the provider requires manual recreation', async () => {
  ...
  expect(await requireManualPluginRefresh({ ...request, appId, connectorName: 'Octo Chat Core', tools: installed, error: 'Recreate the custom app manually.' })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
...
it('keeps an exact app mapping and refuses another installed same-name plugin', async () => {
  ...
  expect(await claimPluginRefresh({ ...b, appId: 'asdk_app_other', connectorName: 'Octo Chat Core', tools })).toBe(false);
});
...
it('requires readable declarations before claiming even an enrolled exact app', async () => {
  ...
  const next = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...next, appId, connectorName: 'Octo Chat Core', tools: null })).toBe(false);
  expect((await readDurable('plugin-refresh') as any[])[0].attempted).toBe(false);
