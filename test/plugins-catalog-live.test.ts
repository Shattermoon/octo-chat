...
for (const id of ['blender', 'unity', 'playwright']) {
  it.runIf(process.env.OCTO_PLUGIN_LIVE_TEST === '1')(`installs ${id} and verifies its advertised tool preview over stdio`, async () => {
    const directory = await makeTempDir(`octo-catalog-${id}-`);
    const client = new Client({ name: 'Octo Chat catalog discovery acceptance', version: '1.0.0' });
    let transport: StdioClientTransport | undefined;
    const page = createServer((_request, response) => { response.setHeader('Content-Type', 'text/html'); response.end('<h1>Octo Chat plugin browser fixture</h1>'); });
    try {
      ...
      if (id === 'playwright') {
        ...
        expect(JSON.stringify(content)).toContain('Octo Chat plugin browser fixture');
      }
    } finally {
