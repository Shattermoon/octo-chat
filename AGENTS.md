# Octo Chat — product logic and agent map
...
| --- | --- |
| Core — `octo-chat-core` | `read`, `view_image`, `find` when command execution is off, `apply_patch`, `exec_command`/`write_stdin`, `download_artifact`, recorded `session`, `update_plan`, `agents`, `session_finish`, code-mode `exec`. |
| Desktop — `octo-chat-desktop` | Windows: 13 Window2 operations, separately permissioned `read_clipboard`/`write_clipboard`, and `exec` with `sky`. Relevant live capabilities are required. |
| Plugins — `octo-chat-plugins` | Enabled external tools with their upstream names and schemas, plus code-mode `exec` when that composition name is available. |
...
plus the explicitly linked project's root `AGENTS.md` inside the existing `OCTO_CONTEXT` frame.
...
with the selected locale in `octo.ui.language`. Changing language repaints owned labels while
...
`octo.plugins.refreshReminder.dismissedVersion`; downloading a newer version does not rearm
...
off. Windows NSIS/Linux AppImage can apply automatically; Linux DEB uses the supported manual
...
CI verifies supported OS families; native `release.yml` builds/smokes four Windows/Linux targets, then
