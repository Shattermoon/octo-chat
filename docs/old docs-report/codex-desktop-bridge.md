...
for those same Desktop threads, and what host-instance guarantees should Octo Chat require
before it exposes them to the model.

## Problem

Octo Chat can inspect local recordings and can control the desktop, but neither is the right
...
thread. Steps 4–6 are still the important product boundary for a model-facing Octo Chat integration.
...
semantics, then wrap only the subset Octo Chat actually needs.
...
The direct `codex queue` experiment has already proven step 3 outside Octo Chat. The full slice closes the
...
2. Is wrapping the shipped exact-thread CLI/app-server surface acceptable, or does the maintainer
   prefer Octo Chat to target a lower-level first-party contract once its read/list guarantees are clear?
