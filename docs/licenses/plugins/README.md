...
These exact upstream texts support the reviewed catalog. They are not the complete dependency notices for an installed plugin: Octo Chat downloads those packages only when the user installs them, and retains their dependency directories and bundled code notices. They do not change the MIT license of Octo Chat. The original Octo Chat SVG illustrations are MIT artwork, not upstream logos.
