...
(() => {
  ...
  const GOAL_MARKER_INSTRUCTION = '\n\nFor this Goal session only: at the end of each final reply, write exactly one separate last line: [[OCTO_GOAL:COMPLETE]] if the entire requested task is finished, or [[OCTO_GOAL:CONTINUE]] if requested work remains. Do not claim completion for partial work. If user input is required, explain it and omit both markers.';
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[Octo Chat: browser observation truncated to fit transport.]';
  ...
  function reportMessages(nowGenerating) {
    ...
    for (const message of rendered) {
      ...
      if (message.role === 'user') {
        ...
        if (!source.canonical && /^\[\[OCTO_CONTEXT:\d{1,6}\]\]/.test(source.text) && CLF_DOM.userPromptText(source.text) === null) continue;
   ...
   *
   * Exact names, never a prefix: `Octo Chat Backup` would be somebody else's
   * connector, and a prefix test would have this app vouch for its traffic.
   */
  const OUR_CONNECTORS = ['Octo Chat Core', 'Octo Chat Desktop', 'TobisComputer'];
  ...
  function controlState(input) {
    ...
    if (disconnected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'Browser connection is disconnected in Octo Chat.',
        action: 'none'
      };
    }
    if (!connected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'Octo Chat is not running on this PC.',
        action: 'none'
      };
    }
    if (!conversationId) {
  ...
  function buildControl() {
    ...
    blocked.setAttribute(
      'data-clf-tip',
      'This chat is blocked in the Octo Chat app: its tool calls are refused and Goal, Loop and auto-compaction are off. To release it, open the app’s Chat tab, hover this chat in the sessions list and press its block symbol.'
    );
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', 'Octo Chat settings');
    root.hidden = true;
  ...
  function renderControl() {
    ...
    control.button.setAttribute('aria-label', 'Octo Chat settings');
  ...
  function replyError(reply) {
    ...
    if (reply.error === 'app_not_found') return 'Octo Chat is not running on this PC.';
  ...
  */
  ...
  window.addEventListener('message', (event) => {
    if (!alive || event.source !== window || event.origin !== location.origin || event.data?.type !== 'octo-usage') return;
  ...
  window.addEventListener('message', (event) => {
    if (!alive || event.source !== window || event.origin !== location.origin || event.data?.type !== 'octo-request-origin') return;
  ...
  window.postMessage({ type: 'octo-usage-request' }, location.origin);
  ...
  function temporaryPlannerPage() {
    ...
    return new URL(location.href).searchParams.get('temporary-chat') === 'true' &&
      (location.href.includes('octo-input=') || desktopDecisionSession?.temporary === true || desktopDecision?.temporary === true);
  }
  ...
  async function acceptDesktopInput(message) {
    ...
    const ownsFreshPage = () => !target && onTarget() && location.pathname === '/' &&
      new URL(location.href).searchParams.get('octo-input') === message.id && !CLF_DOM.turns().length;
  ...
  function ownsPluginRefreshPage(id) {
    ...
    return alive && !generating && !CLF_DOM.generating() && url.pathname === '/' &&
      /^#settings\/Plugins(?:\/plugin_asdk_app_[a-zA-Z0-9_-]+)?$/.test(url.hash) && url.searchParams.get('octo-plugin-refresh') === id;
  }
  ...
  async function refreshManagedPlugin(request) {
    ...
    try {
      ...
      if (new URL(location.href).hash === '#settings/Plugins') {
        ...
        const url = new URL(discovered); url.searchParams.set('octo-plugin-refresh', request.id);
  ...
  function inputReuseSafe() {
    ...
    return alive && !generating && !CLF_DOM.generating() && pendingTools === 0 && !desktopInputBusy &&
      ...
      (home ? !rows.length && !marker.has('octo-input') && !marker.has('temporary-chat') :
  ...
  async function prepareDesktopInputPage(message) {
    ...
    try {
      ...
      const url = new URL(location.href);
      url.searchParams.delete('octo-model-catalog');
      url.searchParams.set('octo-input', message.id);
      url.hash = `octo-input=${message.id}`;
  ...
  function catalogHelper() {
    return !conversationId && location.pathname === '/' &&
      !!new URL(location.href).searchParams.get('octo-model-catalog') && !CLF_DOM.turns().length;
  }
  ...
  if (globalThis.chrome && chrome.runtime && chrome.runtime.onMessage) {
    const runtimeMessage = (message, _sender, sendResponse) => {
      ...
      if (message.type === 'clf-close-temporary-planner') {
        ...
        sendResponse({ safe: temporaryPlannerPage() && location.href.includes(`octo-input=${message.id}`) &&
