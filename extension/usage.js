...
(() => {
  ...
  const project = (data, observedAt, order) => {
    ...
    latestOrder = order;
    latest = { type: 'octo-usage', rows, observedAt }; post(latest, location.origin);
  };
  ...
  async function inspectRequestOrigins(response, observedAt) {
    ...
    const scan = (frame) => {
      ...
      post({ type: 'octo-request-origin', conversationId, requestIds: fresh, observedAt }, location.origin);
    };
    try {
      while (true) {
  ...
  window.addEventListener('message', (event) => {
    if (event.source === window && event.origin === location.origin && event.data?.type === 'octo-usage-request' && latest) post(latest, location.origin);
  });
})();
