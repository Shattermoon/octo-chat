...
async function hello(candidate) {
  try {
    ...
    return body && body.app === 'octo-chat' ? body : null;
  } catch {
    return null;
  }
}
...
async function reconcileBackgroundWindow(policy) {
  ...
  const owns = tab => {
    ...
    try {
      ...
      const inputId = url.searchParams.get('octo-input') || new URLSearchParams(url.hash.slice(1)).get('octo-input');
      ...
      const catalogHelper = url.pathname === '/' && /^[a-f0-9-]{36}$/i.test(url.searchParams.get('octo-model-catalog') || '');
...
async function deliverDesktopInputs(inputs, background, reusableConversations = [], activeIds) {
  ...
  const matchesInput = (input, tab) => {
    ...
    try {
      const url = new URL(tab.pendingUrl || tab.url || '');
      return url.searchParams.get('octo-input') === input.id || new URLSearchParams(url.hash.slice(1)).get('octo-input') === input.id;
    } catch { return false; }
  };
  for (const input of inputs.slice(0, 50)) {
    if (!input || !/^[a-f0-9-]{36}$/i.test(input.id)) continue;
    const target = cleanConversationId(input.conversationId);
    const marker = `octo-input=${encodeURIComponent(input.id)}`;
    ...
    if (input.close === true && input.lifetime === 'temporary-planner') {
      ...
      try {
        ...
        const current = await chrome.tabs.get(tab.id);
        if (proof?.safe === true && ownsDocument(source) && !current.pinned && !current.pendingUrl && new URL(current.url).searchParams.get('octo-input') === input.id &&
...
function pluginRefreshMarker(tab) {
  try { const url = new URL(tab?.pendingUrl || tab?.url || ''); return url.origin === 'https://chatgpt.com' && url.pathname === '/' && /^#settings\/Plugins(?:\/plugin_asdk_app_[a-zA-Z0-9_-]+)?$/.test(url.hash) ? url.searchParams.get('octo-plugin-refresh') : null; } catch { return null; }
}
  ...
  pluginRefreshFlight = (async () => {
    ...
    if (owner && requests.some(request => request.id === owner.id)) {
      ...
      if (pluginRefreshMarker(current) !== owner.id) {
        ...
        url.searchParams.set('octo-plugin-refresh', owner.id);
    ...
    if (!held) {
      ...
      try {
        const tab = await createChatTab(`https://chatgpt.com/?octo-plugin-refresh=${request.id}#settings/Plugins${request.appId ? `/plugin_${request.appId}` : ''}`, background);
...
function catalogTabNonce(tab) {
  try {
    const url = new URL(tab?.pendingUrl || tab?.url || '');
    const nonce = url.searchParams.get('octo-model-catalog');
...
function inspectRequestedModels(request) {
  ...
  modelCatalogFlight = (async () => {
    ...
    if (!tab) {
      ...
      tab = await createChatTab(`https://chatgpt.com/?octo-model-catalog=${wanted.nonce}`, true);
...
const HANDLERS = {
  ...
  async desktop_input(message, sender, source) {
    ...
    if (completed) {
      ...
      if (!conversationId && !String(tab.url || '').includes(`octo-input=${id}`)) return { ok: false };
      ...
      try {
        ...
        if (!current.pinned && !current.pendingUrl && ownsDocument(source) && current.url === tab.url) {
          ...
          if (proof?.safe === true && ownsDocument(source) && !latest.pinned && !latest.pendingUrl && latest.url === tab.url &&
              new URL(latest.url).searchParams.get('octo-input') === id) await chrome.tabs.remove(source.tab);
        }
  ...
  /** Ask every eligible ChatGPT tab to rebuild its Octo Chat activity stream now. */
  async overwriteNow() {
    await load();
